...
public class LinkBlockerEvent implements EventListener {
    ...
    public void onEvent(Event event) {
        if (event instanceof GuildMessageReceivedEvent) {
            ...
            if (main.bypassfilter.contains(((GuildMessageReceivedEvent) event).getAuthor().getId())) {
                main.bypassfilter.remove(((GuildMessageReceivedEvent) event).getAuthor().getId());
                return;
            }
